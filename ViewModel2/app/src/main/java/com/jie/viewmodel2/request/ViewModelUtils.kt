...
object ViewModelUtils {
    fun <VM : ViewModel> createViewModel(
        ...
        val vbClass =
            (activity.javaClass.genericSuperclass as ParameterizedType).actualTypeArguments
    ...
    fun <VM : ViewModel> createViewModel(
        fragment: Fragment, factory: ViewModelProvider.Factory? = null,
        position: Int
    ): VM {
        val vbClass =
            (fragment.javaClass.genericSuperclass as ParameterizedType).actualTypeArguments
                .filterIsInstance<Class<*>>()
        val viewModel = vbClass[position] as Class<VM>
        return factory?.let {
            ViewModelProvider(
