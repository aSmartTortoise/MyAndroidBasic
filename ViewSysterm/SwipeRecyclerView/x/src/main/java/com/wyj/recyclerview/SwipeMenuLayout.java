...
import com.wyj.recyclerview.x.R;
...
public class SwipeMenuLayout extends FrameLayout implements Controller {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        int dy;
        int action = ev.getAction();
        Log.d(TAG, "onTouchEvent: wyj ev.getAction:" + action + " x:" + ev.getX()
        + " y:" + ev.getY());
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                mLastX = (int)ev.getX();
            ...
            case MotionEvent.ACTION_UP: {
                ...
                Log.d(TAG, "onTouchEvent: wyj dx:" + dx + " dy:" + dy
                 + " mScaledMaximumFlingVelocity:" + mScaledMaximumFlingVelocity
                + " mScaledMinimumFlingVelocity:" + mScaledMinimumFlingVelocity);
                mDragging = false;
                mVelocityTracker.computeCurrentVelocity(1000, mScaledMaximumFlingVelocity);
                int velocityX = (int)mVelocityTracker.getXVelocity();
                Log.d(TAG, "onTouchEvent: wyj velocityX:" + velocityX);
                ...
                if (velocity > mScaledMinimumFlingVelocity) {
                    if (mSwipeCurrentHorizontal != null) {
                        ...
                        Log.d(TAG, "onTouchEvent: wyj duration:" + duration);
                        if (mSwipeCurrentHorizontal instanceof RightHorizontal) {
                            if (velocityX < 0) {
                                Log.d(TAG, "onTouchEvent: wyj smoothOpenMenu");
                                smoothOpenMenu(duration);
                            } else {
                                Log.d(TAG, "onTouchEvent: wyj smoothCloseMenu");
    ...
    private void smoothOpenMenu(int duration) {
        if (mSwipeCurrentHorizontal != null) {
            Log.d(TAG, "smoothOpenMenu: wyj autoOpenMenu scrollX:" + getScrollX());
            Log.d(TAG, "smoothOpenMenu: wyj mSwipeCurrentHorizontal:" + mSwipeCurrentHorizontal);
    ...
    public void smoothCloseMenu(int duration) {
        Log.d(TAG, "smoothCloseMenu: wyj mSwipeCurrentHorizontal:" + mSwipeCurrentHorizontal);
        if (mSwipeCurrentHorizontal != null) {
